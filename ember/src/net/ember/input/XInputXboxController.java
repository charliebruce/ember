...
public class XInputXboxController implements Gamepad {
	...
	/**
	 * The previous state of the buttons (to detect changes, and fire events).
	 */
	int prevbuttons=0; 

	/**
	 * The number of times a poll has successively failed.
	 */
	int failCount = 0;
	...
	public void poll(){
		...
		if(sb.get()!=1){
			//If the read fails, we leave the data as-was for the last frame, and just increment the failure counter.
		...
		//If the poll fails 3 consecutive times, we disconnect the controller.
		if(failCount==3){
			Log.warn("Poll of XInput controller "+index+" has failed 3 times. We will not poll any more.");
			connected = false;
		}
	}
	...
	public void rumbleOn(){
		XInput.vibrate(index, 65535, 65535);
	}
	
	
	public void rumbleOff(){
		XInput.vibrate(index, 0, 0);
	}
